...
set_args=(
    ...
    --set "endpoints.oslo_messaging.auth.cyborg.password=$(kubectl --namespace openstack get secret cyborg-rabbitmq-password -o jsonpath='{.data.password}' | base64 -d)"
    ...
    --set "conf.cyborg.keystone_authtoken.memcache_secret_key=$(kubectl --namespace openstack get secret os-memcached -o jsonpath='{.data.memcache_secret_key}' | base64 -d)"
)
